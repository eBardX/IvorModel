...
extension PanMap {
    /// An error thrown by ``PanMap`` operations.
    public enum Error {
...
extension PanMap.Error: EnhancedError {
    ...
    public var message: String {
        ...
        case let .augmentFailure(time):
            "Unable to augment pan map entry, time: \(time)"

        case let .diminishFailure(time):
            "Unable to diminish pan map entry, time: \(time)"
        ...
        case let .moveFailure(time):
            "Unable to move pan map entry, time: \(time)"

        case let .reverseFailure(time):
            "Unable to reverse pan map entry, time: \(time)"
        }
    }
}
...
extension PanMap.Error: Equatable {
}

// MARK: - Sendable

extension PanMap.Error: Sendable {
}
