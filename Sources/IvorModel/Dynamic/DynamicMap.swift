...
extension DynamicMap {
    ...
    /// dynamic level, and extras — carries no information beyond the
    /// original and is silently ignored. Two entries at the same time with
    /// *different* dynamic levels are a deliberate, meaningful step
    ...
    /// Moves the dynamic entry with the given identity to a new time,
    /// keeping its dynamic level and extras, and re-sorts it into place.
    ///
    /// Unlike ``update(entryID:dynamic:extras:)``, this is expected to reorder
    /// entries — that is the point of editing the time itself. Two entries
    /// legitimately ending up at the same time (see
    /// ``insert(time:dynamic:extras:)`` for the discontinuity use case) is
    ...
    /// present — same time, dynamic level, and extras — the same
    ...
    /// Removes the dynamic entry with the given identity, if present.
    ///
    ...
    /// the same combination ``insert(time:dynamic:extras:)`` silently
    ...
    /// - Parameter dynamic:  The new dynamic level for the entry.
