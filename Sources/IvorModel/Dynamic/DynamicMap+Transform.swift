...
extension DynamicMap {
    ...
    ///                         entry in the map.
    ///
    /// - Throws:   ``DynamicMap/Error/invalidAugmentationFactor(_:)`` if `factor` is not a
    ///             rational number ≥ 1; ``DynamicMap/Error/invalidAnchor`` if `anchor` is later
    ///             than the low bound of the entries being augmented; otherwise,
    ///             ``DynamicMap/Error/augmentFailure(_:)`` if an entry cannot be augmented.
    ...
    ///
    /// - Throws:   ``DynamicMap/Error/invalidDiminutionFactor(_:)`` if `factor` is not a rational
    ///             number ≥ 1; ``DynamicMap/Error/invalidAnchor`` if `anchor` is later than the
    ///             low bound of the entries being diminished; otherwise,
    ///             ``DynamicMap/Error/diminishFailure(_:)`` if an entry cannot be diminished.
    ...
    ///                                 every entry in the map.
    ///
    /// - Throws:   ``DynamicMap/Error/moveFailure(_:)`` if an entry cannot be moved.
    ...
    ///                          entry in the map.
    ///
    /// - Throws:   ``DynamicMap/Error/invalidAnchor`` if `timeRange` does not contain the time
    ///             range of the entries being reversed; otherwise,
    ///             ``DynamicMap/Error/reverseFailure(_:)`` if an entry cannot be reversed.
...
extension DynamicMap where TimeType == BeatTime {
