...
extension DynamicMap.EntryID {
    ...
    //
    // Computed, not stored — `EntryID` is nested inside the generic
    // `DynamicMap<TimeType>`, and Swift doesn't allow stored static properties on a
    // type nested in a generic context, even though this one doesn't itself depend
    // on `TimeType`.
    //
